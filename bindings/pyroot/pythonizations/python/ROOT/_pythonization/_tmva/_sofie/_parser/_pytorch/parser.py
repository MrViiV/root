...
from .layers.gru import MakePyTorchGRU
...
RECURRENT_OPS = {"onnx::RNN", "onnx::LSTM", "onnx::GRU", "onnx::BatchNormalization"}
RNN_OPS = {"onnx::RNN", "onnx::LSTM", "onnx::GRU"}
    ...
    def Parse(filename, input_shapes, input_dtypes=None):
        ...
        for node in graph.nodes():
            ...
            try:
                ...
                if node_type in {"onnx::RNN", "onnx::LSTM", "onnx::GRU"}:
                    input_name = list(graph.inputs())[0].debugName()
                    op = parse_fn(node_data, node, weights, rmodel, input_name)
                elif node_type == "onnx::BatchNormalization":
                    op = parse_fn(node_data, node, weights, rmodel)
                else:
        ...
        has_recurrent = any(
             node.kind() in RNN_OPS for node in graph.nodes()
        )
        if has_recurrent:
             for node in graph.nodes():
                 if node.kind() in RNN_OPS:
